...
void PlayGame(Status *status) {
  ...
  while (!status->IsWin()) {
    ...
    for (int i = 0; i < kDirection; ++i) {
      ...
      if (next_status[i] == *status) is_same_status[i] = true;
      next_status[i].PickRandomNumber();
      if (non_zeros != kSize || !(next_status[i] == *status)) is_dead = false;
    }
    ...
    if (!is_same_status[direction]) *status = next_status[direction];
    status->OutputGraph();
    status->OutputPoints();
  }
}
